...
use super::{index::AtomicKey, list::List, Evict, TouchLock};
    ...
    G1::Value: AtomicUsizeValue,
{
    ...
    fn insert(
        ...
        let (inserted, removed) = self.g0.insert(
            ...
            move |p| <G0::Value as AtomicUsizeValue>::from_atomic_usize_ref(&deref(p).inner),
        );
        // XX: why 4
        (
            inserted,
                        ...
                        true => {
                            ...
                            self.g0
                                .remove(
                                    &mut queue.q0,
                                    pointer,
                                    |p| <G0::Value as AtomicUsizeValue>::from_atomic_usize_ref(&(&deref)(p).inner)
                                );
                            let (_pointer, removed) =
                                self.g1
                                    .insert(
                                        &mut queue.q1,
                                        |v1| {
                                        ...
                                        |p| <G1::Value as AtomicUsizeValue>::from_atomic_usize_ref(&(&deref)(p).inner)
                                    );
